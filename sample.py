...
def parse_args():
    ...
    parser.add_argument("--ckpt_path", type=str, default='./logs/SmoothL1_64_6K_0.001_5to0/checkpoints/None_100000.pth.tar')
    parser.add_argument("--group", type=int, default=4)
    parser.add_argument("--sample_dir", type=str, default='./sample')
    parser.add_argument("--test_data_dir", type=str, default="../Set5")
    parser.add_argument("--scale1", type=int, default=4)
...
def sample(net, dataset, cfg):
    ...
    for step, (hr, lr, name) in enumerate(dataset):
        ...
        with torch.no_grad():
            ...
            sr = net(lr_patch, cfg.scale1, cfg.scale2)
        ...
        ti = t2 - t1
        print("Saved {} ({}x{} -> {}x{}, {:.3f}s -- PSNR {} )".format(sr_im_path, lr.shape[1], lr.shape[2], sr.shape[1], sr.shape[2], t2 - t1, psnr))
    print('Average PSNR on scale X{} is {} '.format(cfg.scale1, avg_psnr))



def main(cfg):
    module = importlib.import_module("{}".format(cfg.model))
    net = module.Network(group=cfg.group)
    ...
    net = net.to(device)
    net.set_scale(cfg.scale1, cfg.scale2)

    dataset = TestDataset(cfg.test_data_dir, cfg.scale1, False)
